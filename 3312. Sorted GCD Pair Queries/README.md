...
<p>Let <code>gcdPairs</code> denote an array obtained by calculating the <span class="cursor-pointer relative text-dark-blue-s text-sm" data-keyword="gcd-function"><button aria-controls="radix-_r_1l_" aria-expanded="false" aria-haspopup="dialog" class="" data-state="closed" type="button">GCD</button></span> of all possible pairs <code>(nums[i], nums[j])</code>, where <code>0 &lt;= i &lt; j &lt; n</code>, and then sorting these values in <strong>ascending</strong> order.</p>
